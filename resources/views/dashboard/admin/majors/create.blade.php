...
<div class="container mx-auto px-6 py-8">
    <div class="bg-white rounded-lg shadow-xl p-8 max-w-2xl mx-auto">
        <h2 class="text-3xl font-extrabold text-gray-800 mb-6 text-center border-b pb-4">Tambah Jurusan Baru</h2>
        ...
        <form action="{{ route('majors.store') }}" method="POST">
            ...
            <div class="mb-8">
                ...
                <textarea name="description" id="description" rows="4"
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-yellow-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
