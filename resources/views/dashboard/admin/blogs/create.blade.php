...
<div class="container mx-auto px-6 py-8">
    <div class="bg-white rounded-lg shadow-xl p-8 max-w-3xl mx-auto border border-gray-200">
        <h2 class="text-3xl font-extrabold text-gray-800 mb-8 text-center border-b pb-4">Buat Blog Baru</h2>
        ...
        {{-- Menggunakan route('blogs.store') sesuai dengan definisi resource route Anda --}}
        <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="title" id="title"
                       ...
                       value="{{ old('title') }}" required placeholder="Masukkan judul blog">
                @error('title')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="8"
                          ...
                          required placeholder="Tulis konten blog di sini...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <label for="comments_enabled" class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="comments_enabled" name="comments_enabled" value="1" class="sr-only peer" checked> {{-- Default aktif --}}
            ...
            <div class="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
                <button type="submit"
                        class="inline-flex items-center px-8 py-3 bg-yellow-600 text-white font-semibold rounded-full shadow-lg
                               hover:bg-yellow-700 transition duration-300 transform hover:scale-105 group">
                    <i class="fas fa-save mr-2 group-hover:rotate-6 transition-transform duration-300"></i> Simpan Blog
                </button>
