...
@section('content')
{{-- SweetAlert2 CDN diasumsikan dimuat di layouts/alumnidashboard.blade.php --}}

<div class="container mx-auto px-6 py-10">
    <div class="bg-white rounded-2xl shadow-2xl p-8 md:p-12 max-w-6xl mx-auto border border-gray-100 transform transition-all duration-500 hover:scale-[1.005] hover:shadow-3xl">
        <h2 class="text-5xl font-extrabold text-gray-900 mb-10 text-center tracking-tight leading-tight border-b-4 border-blue-500 pb-5">
            {{ $blog->title }}
        </h2>
        ...
        <div class="flex flex-col md:flex-row items-start md:space-x-10">
            ...
            @if ($blog->image)
                <div class="md:w-1/2 mb-8 md:mb-0 flex-shrink-0 relative group">
                    <img src="{{ asset('storage/' . $blog->image) }}"
                         class="w-full h-auto object-cover rounded-2xl shadow-xl border-4 border-blue-400 transition-transform duration-500 hover:scale-[1.03]"
                         alt="Blog Image">
            ...
            <div class="md:w-1/2 flex-grow">
                <div class="flex flex-wrap items-center text-gray-700 text-base mb-6 space-x-6">
                    ...
                    <span class="flex items-center">
                        <i class="fas fa-comments text-blue-600 mr-2 text-xl"></i>
                        Komentar: <span class="font-semibold ml-1">{{ $blog->comments_enabled ? 'Aktif' : 'Nonaktif' }}</span>
                    </span>
                </div>

                <div class="prose prose-xl text-gray-800 text-justify leading-relaxed mb-10 border-l-4 border-blue-400 pl-4 py-2">
                    {!! nl2br(e($blog->description)) !!}
                </div>

                {{-- Actions --}}
                <div class="flex justify-start mt-10 pt-6 border-t border-gray-200">
                    <a href="{{ route('alumni.blogs.index') }}"
                       class="inline-flex items-center px-8 py-4 bg-gray-300 text-gray-800 font-semibold rounded-full shadow-lg
                        ...
                        <i class="fas fa-arrow-left mr-3 text-xl"></i> Kembali ke Daftar Blog
                    </a>
                </div>
            </div>
        </div>
        ...
        @if($blog->comments_enabled)
            <div class="mt-12 pt-8 border-t-2 border-gray-200">
                <h3 class="text-3xl font-bold text-gray-900 mb-6 border-b pb-4">
                ...
                <div class="bg-gray-50 p-6 rounded-lg shadow-sm mb-8 border border-gray-200">
                    <h4 class="text-xl font-semibold text-gray-800 mb-4">Tulis Komentar Anda</h4>
                    {{-- Menggunakan route('alumni.comments.store') sesuai yang kita definisikan --}}
                    <form action="{{ route('alumni.comments.store', $blog) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <textarea name="content" rows="4"
                                      class="shadow-sm appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200 @error('content') border-red-500 @enderror"
                                      placeholder="Tulis komentar Anda di sini..." required>{{ old('content') }}</textarea>
                            @error('content')
                ...
                @forelse($blog->comments as $comment)
                    <div class="bg-white p-6 rounded-lg shadow-md mb-4 border border-gray-200 relative">
                        <div class="flex items-center mb-2">
                            <div class="flex-shrink-0">
                                {{-- Avatar User --}}
                                @if($comment->user->profile_picture)
                                    <img src="{{ asset('storage/' . $comment->user->profile_picture) }}" alt="{{ $comment->user->name }}" class="w-10 h-10 rounded-full object-cover border-2 border-blue-300">
                                @else
                                    <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-xl font-bold">
                                        {{ $comment->user->name[0] }}
                                    </div>
                                @endif
                            </div>
                        ...
                        <p class="text-gray-700 leading-relaxed mt-3">{{ $comment->content }}</p>
                        ...
                        <div class="flex items-center mt-4 text-sm text-gray-500 space-x-4">
                            ...
                            @if(Auth::id() == $comment->user_id) {{-- Jika pemilik komentar --}}
                                <form action="{{ route('alumni.comments.destroy', $comment) }}" method="POST" onsubmit="return confirmDeleteComment(event);" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center hover:text-red-600 transition-colors duration-200">
                        ...
                        <div id="replyForm{{ $comment->id }}" class="hidden mt-4 pl-4 border-l-2 border-gray-100">
                            ...
                            <form action="{{ route('alumni.comments.reply', $comment) }}" method="POST">
                                @csrf
                        ...
                        {{-- Tampilkan Balasan --}}
                        @include('components.comments_replies_alumni', ['replies' => $comment->replies])
                    </div>
                @empty
                    <p class="text-gray-600 text-center py-4">Belum ada komentar untuk blog ini. Jadilah yang pertama berkomentar!</p>
                @endforelse
            </div>
        @else
...
<script>
    ...
    // Script untuk SweetAlert2 konfirmasi hapus komentar
