{{-- resources/views/components/comments_replies_alumni.blade.php --}}
@if ($replies->count())
    <div class="ms-8 mt-3 border-l-2 border-gray-200 pl-4"> {{-- Indent balasan lebih dalam --}}
        @foreach($replies as $reply)
            <div class="bg-gray-100 p-4 rounded-lg shadow-xs mb-2 border border-gray-200 relative">
                ...
                <div class="flex items-center mt-3 text-sm text-gray-500 space-x-4">
                    ...
                    @if(Auth::id() == $reply->user_id) {{-- Jika pemilik balasan --}}
                        <form action="{{ route('alumni.comments.destroy', $reply) }}" method="POST" onsubmit="return confirmDeleteComment(event);" class="inline-block">
                            @csrf
                            @method('DELETE')
                ...
                <div id="replyForm{{ $reply->id }}" class="hidden mt-4 pl-4 border-l-2 border-gray-100">
                    ...
                    <form action="{{ route('alumni.comments.reply', $reply) }}" method="POST">
                        @csrf
                ...
                @if ($reply->replies->count())
                    @include('components.comments_replies_alumni', ['replies' => $reply->replies])
                @endif
            </div>
        @endforeach
    </div>
