...
<body class="bg-gray-100 font-sans antialiased">
            ...
            <ul class="space-y-3 flex-grow">
                ...
                <li>
                    <a href="{{ route('users.index') }}"
                        class="sidebar-link flex items-center p-3 rounded-lg transition-colors duration-200
                                {{ request()->routeIs('users.index') || request()->routeIs('users.create') || request()->routeIs('users.edit') || request()->routeIs('users.show') ? 'active' : '' }}">
                ...
                <li>
                    <a href="{{ route('majors.index') }}"
                        class="sidebar-link flex items-center p-3 rounded-lg transition-colors duration-200
                                {{ request()->routeIs('majors.index') || request()->routeIs('majors.create') || request()->routeIs('majors.edit') || request()->routeIs('majors.show') ? 'active' : '' }}">
                        <i class="fas fa-graduation-cap mr-3 text-lg w-6 text-center"></i>
                ...
                <li>
                    <a href="{{ route('about.index') }}"
                        class="sidebar-link flex items-center p-3 rounded-lg transition-colors duration-200
                                {{ request()->routeIs('about.index') || request()->routeIs('about.edit') ? 'active' : '' }}">
                        <i class="fas fa-info-circle mr-3 text-lg w-6 text-center"></i>
                ...
                <li>
                    <a href="{{ route('events.index') }}"
                        class="sidebar-link flex items-center p-3 rounded-lg transition-colors duration-200
                                {{ request()->routeIs('events.index') || request()->routeIs('events.create') || request()->routeIs('events.edit') || request()->routeIs('events.show') ? 'active' : '' }}">
                ...
                <li>
                    <a href="/admin/announcements"
                        class="sidebar-link flex items-center p-3 rounded-lg transition-colors duration-200
                                {{ request()->is('admin/announcements*') ? 'active' : '' }}">
            ...
            <nav class="bg-gray-900 shadow-xl p-4 md:px-8 flex justify-between items-center w-full">
                ...
                <div class="flex items-center">
                    ...
                    <a href="#" class="text-white text-2xl font-bold tracking-tight transform transition-transform duration-300 hover:scale-105">
                        <i class="fas fa-graduation-cap text-yellow-400 mr-3"></i> AlumniGarnissa
                    </a>
                </div>
    ...
    <div class="md:hidden fixed bottom-4 right-4 z-30">
        <button id="mobile-menu-toggle" class="p-3 bg-gray-800 text-white rounded-full shadow-lg transition-all duration-300 transform hover:scale-110">
            <i class="fas fa-bars text-xl"></i>
        </button>
    </div>
    ...
    <script>
        ...
        const sidebarOverlay = document.getElementById('sidebar-overlay'); // Ambil elemen overlay
        ...
        function toggleSidebarText(hide) {
            ...
            if (hide) {
                ...
                sidebar.classList.add('w-20', 'px-2'); // Lebar lebih kecil untuk ikon saja
                mainContentArea.classList.remove('md:ml-64');
                mainContentArea.classList.add('md:ml-20');
                sidebar.classList.add('sidebar-hidden'); // Menambahkan kelas untuk styling tambahan
                // Sembunyikan judul "Admin Panel" saat sidebar disembunyikan
                ...
                sidebar.classList.remove('sidebar-hidden'); // Menghapus kelas
                // Tampilkan kembali judul "Admin Panel"
        ...
        function closeMobileSidebar() {
            ...
            document.body.classList.remove('overflow-hidden'); // Izinkan scrolling lagi
        }

        // Mobile menu toggle functionality
        mobileMenuToggle.addEventListener('click', function() {
            sidebar.classList.toggle('-translate-x-full');
            sidebarOverlay.classList.toggle('hidden'); // Toggle overlay
            document.body.classList.toggle('overflow-hidden'); // Cegah scrolling body
            // Pastikan sidebar dalam mode teks penuh saat dibuka di mobile
        ...
        window.addEventListener('resize', () => {
            if (window.innerWidth >= 768) { // md breakpoint
                // Jika sebelumnya tersembunyi di mobile, pastikan muncul kembali di desktop
                sidebar.classList.remove('-translate-x-full');
                sidebarOverlay.classList.add('hidden'); // Sembunyikan overlay di desktop
                document.body.classList.remove('overflow-hidden'); // Izinkan scrolling
                // Atur ulang ke mode tampilan penuh jika sebelumnya disembunyikan oleh toggle desktop
                // Hanya reset ke tampilan penuh jika sidebar tidak dalam mode hidden oleh toggle desktop
                if (!sidebar.classList.contains('sidebar-hidden')) {
                     toggleSidebarText(false);
                }
            } else {
                // Pastikan sidebar kembali tersembunyi jika di-resize ke mobile
                sidebar.classList.add('-translate-x-full');
                sidebarOverlay.classList.add('hidden'); // Sembunyikan overlay di mobile
                document.body.classList.remove('overflow-hidden'); // Izinkan scrolling
                // Pastikan teks sidebar terlihat saat kembali ke mobile jika sebelumnya disembunyikan
                toggleSidebarText(false); // Reset ke tampilan penuh saat di mobile
            }
        });

        // Inisialisasi tampilan sidebar saat pertama kali dimuat di desktop
        // Jika Anda ingin sidebar defaultnya tersembunyi (hanya ikon) di desktop, uncomment baris di bawah ini
        // window.addEventListener('DOMContentLoaded', () => {
        //     if (window.innerWidth >= 768) {
        //         toggleSidebarText(true); // Sembunyikan teks secara default di desktop
        //     }
        // });
    </script>
</body>
